...
namespace EHR_API.Controllers
{
    ...
    public class PersonalDataAPIController : ControllerBase
    {
        ...
        public PersonalDataController(IMainRepository db, IMapper mapper)
        {
            _db = db;
            _mapper = mapper;
            _response = new();
        ...
        public async Task<ActionResult<APIResponse>> GetUserPersonalData(string id)
        {
            try
            {
                ...
                var entity = await _db._personal.GetAsync(expression: g => g.Id == id, includeProperties: "Governorate, RegistrationData");
                if (entity == null)
                {
        ...
        [HttpDelete("{id}", Name = "DeleteUserPersonalData")]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<ActionResult<APIResponse>> DeleteUserPersonalData(string id) 
        {
            try
            {
                if (id == null)
